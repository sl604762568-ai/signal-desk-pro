...
class DirectPublicProvider:
    ...
    def fetch(self, fast: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            "updated_at":now.isoformat(timespec="seconds"),
            "market_status":_market_status(now),
            "zt_count":(int(zt_count) if zt_count is not None else None),
            "zb_count":(int(zb_count) if zb_count is not None else None),
            "dt_count":(int(dt_count) if dt_count is not None else None),"seal_rate":seal_rate,
            "market_stats_valid":bool(full_market),
