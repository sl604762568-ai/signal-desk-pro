...
把本压缩包解压后的全部文件上传到原 GitHub 仓库并覆盖同名文件，提交 Commit。Render 已连接该仓库时会自动重新部署。部署完成后首页会出现 **AI复盘选股** 区域。

复盘按钮会请求 `/api/review-picks`。第一次扫描要逐只读取历史日线，云端可能需要几十秒；若第三方数据源限流，稍后重试即可。
